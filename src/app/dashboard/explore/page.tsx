...
import { Search, ChevronDown, Users, FileText, Group, Gamepad2, Plus } from 'lucide-react';
...
const SocialExplorePage = () => {
  ...
  const UserCard = ({ user }: { user: User }) => (
    <div className="bg-white rounded-lg shadow-sm border overflow-hidden">
      ...
      <div className="relative h-32 bg-gradient-to-br from-purple-900 via-blue-900 to-black overflow-hidden">
        {/* Starry background effect */}
        <div className="absolute inset-0 opacity-60">
          <div className="absolute top-2 left-4 w-1 h-1 bg-white rounded-full"></div>
          <div className="absolute top-6 right-8 w-0.5 h-0.5 bg-white rounded-full"></div>
          <div className="absolute top-8 left-12 w-1 h-1 bg-white rounded-full"></div>
          <div className="absolute top-12 right-4 w-0.5 h-0.5 bg-white rounded-full"></div>
          <div className="absolute top-16 left-8 w-1 h-1 bg-white rounded-full"></div>
          <div className="absolute top-20 right-12 w-0.5 h-0.5 bg-white rounded-full"></div>
          <div className="absolute bottom-8 left-6 w-1 h-1 bg-white rounded-full"></div>
          <div className="absolute bottom-12 right-6 w-0.5 h-0.5 bg-white rounded-full"></div>
          <div className="absolute bottom-16 left-16 w-1 h-1 bg-white rounded-full"></div>
          {/* Nebula effect */}
          <div className="absolute top-4 right-6 w-8 h-8 bg-purple-400 rounded-full opacity-30 blur-sm"></div>
          <div className="absolute bottom-6 left-8 w-6 h-6 bg-pink-400 rounded-full opacity-20 blur-sm"></div>
        </div>
        
        {/* Centered Avatar */}
        <div className="absolute inset-0 flex items-center justify-center">
          <div className="w-16 h-16 rounded-full bg-orange-200 flex items-center justify-center border-2 border-white shadow-lg">
            {user.isGame ? (
      ...
      <div className="p-4 text-center">
        <h3 className="font-medium text-gray-900 text-sm mb-3 truncate">
          {user.username}
        </h3>
        <button
          onClick={() => handleFollow(user.id)}
          className="w-full bg-blue-100 text-blue-600 py-2 px-4 rounded-md text-sm font-medium hover:bg-blue-200 transition-colors"
        >
          Follow
        </button>
      </div>
  ...
  const PageListItem = ({ page }: { page: Page }) => (
    <div className="bg-white border-b last:border-b-0 p-4 flex items-center justify-between hover:bg-gray-50 transition-colors">
      <div className="flex items-center gap-4">
        {/* Page Icon */}
        <div className={`w-12 h-12 rounded-full flex items-center justify-center ${page.iconColor}`}>
          {page.icon === '📄' ? (
            <FileText className="w-6 h-6 text-orange-600" />
          ) : page.icon === '🎨' ? (
            <div className="w-8 h-8 rounded-full bg-gradient-to-br from-blue-400 via-purple-500 to-yellow-500"></div>
          ) : (
            <span className="font-bold text-lg">{page.icon}</span>
          )}
        </div>
        
        {/* Page Info */}
        <div>
          <h3 className="font-medium text-gray-900 text-base mb-1">{page.name}</h3>
    ...
    <div className="bg-white rounded-lg shadow-sm border">
      ...
      <div className="p-4 text-center border-t">
        <button
          ...
          className="flex items-center gap-2 text-blue-600 font-medium hover:text-blue-700 transition-colors mx-auto"
        >
      ...
      <div className="flex justify-center mb-4">
        <div className="relative max-w-md w-full">
          <input
            ...
            placeholder="Keyword"
            className="w-full pl-4 pr-12 py-2 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent text-center"
          />
          <button
            onClick={handleSearch}
            className="absolute right-2 top-1/2 transform -translate-y-1/2 w-7 h-7 bg-blue-500 text-white rounded-lg flex items-center justify-center hover:bg-blue-600 transition-colors"
          >
        ...
        {Object.entries(filters).map(([key, value]) => (
          <div key={key} className="relative">
            <select
              ...
              className="appearance-none bg-white border border-gray-200 rounded-md px-3 py-1 pr-6 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent text-xs"
            >
    ...
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-6xl mx-auto px-6 py-6">
        ...
        {(activeTab === 'Groups' || activeTab === 'Games') && (
          <div className="bg-white rounded-lg shadow-sm border min-h-96 flex flex-col items-center justify-center p-8">
            <div className="w-16 h-16 bg-gray-100 rounded-lg flex items-center justify-center mb-4">
              {activeTab === 'Groups' && <Group className="w-8 h-8 text-gray-500" />}
              {activeTab === 'Games' && <Gamepad2 className="w-8 h-8 text-gray-500" />}
            </div>
            <p className="text-gray-600 font-medium">{activeTab} coming soon</p>
          </div>
        )}
