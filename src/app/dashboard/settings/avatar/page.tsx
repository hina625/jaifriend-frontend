  ...
  useEffect(() => {
    const fetchUserImages = async () => {
      try {
        ...
        const apiUrl = `${process.env.NEXT_PUBLIC_API_URL || 'https://jaifriend-bacnd-production.up.railway.app'}/api/userimages`;
  ...
  const handleImageUpload = async (type: 'avatar' | 'cover', file: File) => {
    ...
    try {
      ...
      formData.append(type, file);
              ...
              const apiUrl = `${process.env.NEXT_PUBLIC_API_URL || 'https://jaifriend-bacnd-production.up.railway.app'}/api/userimages/${type}`;
    ...
    try {
      ...
      if (!token) {
        ...
        return;
              ...
              const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'https://jaifriend-bacnd-production.up.railway.app'}/api/userimages/${type}`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${token}`
        }
