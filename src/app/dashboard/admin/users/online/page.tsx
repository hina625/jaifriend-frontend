...
const OnlineUsersPage: React.FC = () => {
  ...
  const fetchOnlineUsers = async (page = 1) => {
    try {
      ...
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL || 'https://jaifriend-bacnd-production.up.railway.app'}/api/admin/users/online?page=${page}&limit=20&sortBy=${sortField}&sortOrder=${sortOrder}`,
        {
  ...
  const handleBulkAction = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL || 'https://jaifriend-bacnd-production.up.railway.app'}/api/admin/users/bulk/${bulkAction}`,
        {
          method: 'POST',
          headers: {
