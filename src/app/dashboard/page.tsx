    ...
    <div className="bg-[#f4f7fb] min-h-screen pt-4 pb-6 w-full">
        ...
        {user && (
          <div className="flex items-center gap-2 mb-4 px-2 md:px-0">
            <img src={user.avatar || '/avatars/1.png.png'} className="w-10 h-10 rounded-full border-2 border-blue-500" />
            <span className="font-bold text-blue-700 text-xs md:text-base">ID: {user._id || user.id}</span>
          </div>
        )}
      {/* Stories Row at the top */}
        <div className="w-full pt-2 px-2 sm:px-4">
          <div className="flex gap-2 mb-4 overflow-x-auto pb-2">
          {/* Current user story */}
            <div className="flex flex-col items-center group cursor-pointer" onClick={() => storyInputRef.current?.click()}>
              {userStory ? (
                userStory.startsWith('data:video') ? (
                  <video
                    src={userStory}
                    className="w-32 h-48 rounded-2xl border-4 border-blue-500 mb-3 shadow-xl object-cover transition-transform group-hover:scale-105"
                    controls
                  />
                ) : (
                  <img
            ...
            <span className="text-xs text-[#022e8a] font-semibold">Your Story</span>
              <input
                type="file"
                accept="image/*,video/*"
                className="hidden"
            ...
            {/* Demo static stories - show only blank/default box, no avatar */}
          {[1,2,3,4,5,6].map((i) => (
            <div key={i} className="flex flex-col items-center group cursor-pointer">
                <div className="w-32 h-48 rounded-2xl border-4 border-gray-300 mb-3 shadow-xl bg-gray-200 flex items-center justify-center transition-transform group-hover:scale-105">
                  {/* Optionally, you can put a user icon or plus icon here */}
                  {/* <span className='text-3xl text-gray-400'>+</span> */}
                </div>
              <span className="text-xs text-[#34495e] group-hover:text-[#022e8a] font-medium">User {i}</span>
            </div>
          ))}
        </div>
      </div>
        {/* Main 3-column layout - Responsive */}
        <div className="flex flex-col lg:flex-row px-2 sm:px-4 gap-3 w-full">
          <div className="w-full lg:flex-1 max-w-2xl mx-auto">
          {/* Post Creation - Reduced padding */}
              ...
              {(() => {
                ...
                return combinedFeed.map((item: any) => {
                  if (item.type === 'album') {
                    ...
                    return (
                      <div key={item._id || item.id} className="bg-white rounded-xl shadow p-2 mb-3">
                        <div className="flex items-center gap-2 mb-2">
                              ...
                              {item.user ? (
                                <Link href={`/dashboard/profile/${item.user._id || item.user.id || item.user.userId}`}>
                                  <img
                                    ...
                                    className="w-8 h-8 rounded-full border-2 border-blue-400 mr-2 cursor-pointer"
                                ...
                                <img
                                  ...
                                  className="w-8 h-8 rounded-full border-2 border-blue-400 mr-2"
                                  alt="User"
                                />
                              )}
                              {item.user ? (
                                <Link href={`/dashboard/profile/${item.user._id || item.user.id || item.user.userId}`} className="font-semibold text-sm hover:underline cursor-pointer">
                          ...
                          {false && (
                            <div className="flex gap-1 ml-4">
                              <button
                                className="text-xs text-blue-600 border border-blue-600 px-2 py-1 rounded hover:bg-blue-50 transition"
                                onClick={() => startEditPost(item)}
                              >
                                ✏️ Edit
                              </button>
                              <button
                                className="text-xs text-red-600 border border-red-600 px-2 py-1 rounded hover:bg-red-50 transition"
                        ...
                        {item.media && item.media.length > 0 ? (
                          <div className="mb-2">
                            {item.media.length === 1 ? (
                              ...
                              item.media[0].type === 'video' ? (
                                ...
                                <img src={getMediaUrl(item.media[0].url)} alt="media" className="rounded-lg w-full max-h-64 object-cover" />
                              )
                            ) : (
                              // Multiple media - grid layout
                              <div className={`grid gap-1 ${item.media.length === 2 ? 'grid-cols-2' : 'grid-cols-3'}`}>
                                  ...
                                  <div key={index} className="relative">
                                    {media.type === 'video' ? (
                                      <video 
                                        ...
                                        onClick={() => {
                                          // Handle video click - could open in modal
                          ...
                          item.media.type === 'video' ? (
                            ...
                            <img src={getMediaUrl(item.media.url)} alt="media" className="rounded-lg w-full max-h-64 object-cover" />
                          )
                        ) : item.image && item.image !== '' ? (
                          // Legacy image support
                          <img src={getMediaUrl(item.image)} alt="media" className="rounded-lg w-full max-h-64 object-cover" />
                        ) : null}
                        <div className="flex gap-2 mt-2">
                          <button 
                            className={`flex items-center gap-1 px-2 py-1 rounded-lg transition-all duration-200 text-sm ${
                          ...
                          >
                            <span>🔗</span>
                            <span className="text-xs font-medium">
                              Share {Array.isArray(item.shares) ? `(${item.shares.length})` : ''}
                            </span>
                          </button>
                          <span className="flex items-center gap-1 px-2 py-1 text-gray-400 text-sm">
                            <span>💬</span>
                            <span className="text-xs font-medium">
                              {Array.isArray(item.comments) ? item.comments.length : 0} comments
                            </span>
                          </span>
                        </div>
                        {/* Comments Section */}
                            ...
                            <div className="space-y-2 mb-2">
                              {item.comments.map((comment: any, idx: number) => (
                                <div key={idx} className="flex items-start gap-2">
                                  <img src={comment.user?.avatar || '/avatars/1.png.png'} alt="avatar" className="w-6 h-6 rounded-full" />
                                  <div>
                                    <span className="font-medium text-xs">{comment.user?.name || 'Anonymous'}</span>
                                    <span className="ml-2 text-xs text-gray-400">{comment.createdAt ? new Date(comment.createdAt).toLocaleString() : ''}</span>
                                    <div className="text-sm inline-block mr-2">{comment.text}</div>
                                    {/* Delete button for comment owner or post owner */}
                                    {false && (
                                      <button
                                        className="text-xs text-red-400 hover:underline ml-2"
                                        onClick={() => handleDeleteComment(item._id || item.id, comment._id)}
                                      >
                                        Delete
                                      </button>
                                    )}
          ...
          <div className="w-full lg:w-1/4 flex flex-col gap-4 mt-4 lg:mt-0">
          ...
          <div className="bg-white rounded-xl shadow p-2">
            ...
            <div className="grid grid-cols-2 gap-2">
              <img src="https://images.unsplash.com/photo-1519125323398-675f0ddb6308" className="rounded-lg" />
              <img src="https://images.unsplash.com/photo-1506744038136-46273834b3fb" className="rounded-lg" />
            </div>
          </div>
        </div>
      </div>
