...
const User1Profile = () => {
  ...
  const fetchUserProfile = async () => {
    try {
      ...
      const response = await fetch(process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5000/api/users/search?q=ahmed_khan', {  
        headers: {
      ...
      if (response.ok) {
        ...
        if (userData) {
          ...
          const userResponse = await fetch(process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5000/api/users/${userData.id}', { 
            headers: {
              'Authorization': `Bearer ${token}`
            }
          });
  ...
  const fetchUserPosts = async () => {
    try {
      ...
      const searchResponse = await fetch(process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5000/api/users/search?q=ahmed_khan', { 
        headers: {
          'Authorization': `Bearer ${token}`
        }
      });
      ...
      if (searchResponse.ok) {
        ...
        if (userData) {
          const postsResponse = await fetch(process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5000/api/users/${userData.id}/posts', { 
            headers: {
              'Authorization': `Bearer ${token}`
            }
          });
  ...
  const handleFollow = async () => {
    try {
      ...
      const response = await fetch(process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5000/api/users/${user.id}/follow', { 
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`
        }
  ...
  const handleBlock = async () => {
    try {
      ...
      const response = await fetch(process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5000/api/users/${user.id}/block', { 
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`
        }
