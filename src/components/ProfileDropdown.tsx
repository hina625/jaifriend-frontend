...
export default function ProfileDropdown({ profile }: ProfileDropdownProps) {
  return (
    <div className="absolute right-0 top-12 w-80 bg-white rounded-2xl shadow-2xl z-50 p-4 flex flex-col gap-2 border border-gray-100">
      {/* Profile Section */}
      <div className="flex items-center gap-3 mb-2">
        <img
          src={profile.avatar || "/avatars/1.png.png"}
          alt="avatar"
          className="w-12 h-12 rounded-full border border-gray-200 object-cover"
        />
        <div className="flex flex-col">
          <span className="font-semibold text-base">{profile.fullName || 'My Profile'}</span>
      ...
      <div className="flex flex-col gap-1 divide-y divide-gray-100">
        <button className="flex items-center gap-3 py-2 hover:bg-gray-50 rounded-lg px-2 text-left">
          <span className="bg-gray-100 p-2 rounded-full text-lg">🔄</span>
          <span className="font-medium">Switch Account</span>
        </button>
        <div className="py-1" />
        <button className="flex items-center gap-3 py-2 hover:bg-gray-50 rounded-lg px-2 text-left">
          <span className="bg-gray-100 p-2 rounded-full text-lg">🛠️</span>
